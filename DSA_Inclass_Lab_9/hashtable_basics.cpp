...
struct HashTable{
    ...
    int hashfunc(string user_name){
        ...
        int hash = 0;
	//add your code below
    //Calculate the sum of ASCII values of the characters in the user name
        for (char c : user_name){
             sum += (int)c; // convert char to int and add to sum
    }
	...
	//add an if condition to complete the code here
        if(empty){
            ...
            cout<<"Hash collision detected, unable to save password"<<endl;
    }
	...
	//add an if condition to complete the code here
        if(empty){
            cout<<"No password found for user "<<user_name<<"\n";
             ...
             cout<<"Password ofr user "<<user_name<<": "<<password[hash]<<endl; 
        }
